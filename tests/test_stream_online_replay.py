...
def _build_retargeted_run(run_dir: Path):
    run_dir.mkdir(parents=True, exist_ok=True)
    condition = {
        "print_pressure_psi": 1.61,
        "print_pulse_width_us": 1500,
    ...
    tail_plan = {
        ...
        "refine_step_us": 50,
        "refine_replicates": 1,
        "planned_coarse_delay_count": 4,
        "reserved_refine_delay_count": 1,
        "reserved_refine_capture_count": 1,
        "plan_source": "exact_prior_minus_lead",
        "tail_retarget_count": 1,
        "retargeted_coarse_start_delay_us": 6800,
    ...
    tail_rows = [
        ...
        online_cal_mod.build_online_stream_frame_row(
            ...
            status="accepted",
            delay_us=6900,
            delay_from_emergence_us=3700,
            replicate_index=1,
            qc={"tail_qc_pass": True},
            image_ref={"capture_id": "tail_c_03"},
            warnings=[],
            attached_width_px=73.5,
        ),
        online_cal_mod.build_online_stream_frame_row(
            ...
            status="accepted",
            delay_us=7100,
            delay_from_emergence_us=3900,
            replicate_index=1,
            qc={"tail_qc_pass": True},
            image_ref={"capture_id": "tail_c_04"},
            warnings=[],
            attached_width_px=66.0,
        ),
        online_cal_mod.build_online_stream_frame_row(
    ...
    refine_summaries = [
        online_tail_mod.summarize_online_stream_tail_delay([tail_rows[4]], baseline_width_px=flow_fit["steady_width_baseline_px"]),
    ]
    tail_result = online_tail_mod.resolve_online_stream_tail_result(
        flow_fit_result=flow_fit,
        tail_plan=tail_plan,
...
def test_replay_online_stream_run_matches_retargeted_tail_artifacts(tmp_path):
    run_dir = tmp_path / "run_01"
    _build_retargeted_run(run_dir)

    report = online_replay_mod.replay_online_stream_run(run_dir)

    assert report["comparison"]["all_match"] is True
    assert report["replayed"]["tail_result"]["tail_phase"]["tail_retarget_count"] == 1
    assert report["replayed"]["tail_result"]["tail_phase"]["retargeted_coarse_start_delay_us"] == 6800
