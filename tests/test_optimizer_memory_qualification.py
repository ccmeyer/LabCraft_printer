...
@pytest.mark.skipif(sys.platform != "win32", reason="Windows ctypes sampler")
def test_repeated_sampling_does_not_accumulate_native_interface_types():
    import ctypes
    memory.memory_sample(os.getpid())
    before = len(ctypes._pointer_type_cache)
    for _ in range(100): memory.memory_sample(os.getpid())
    assert len(ctypes._pointer_type_cache) == before


def test_process_ownership_rejects_unrelated_pid():
    assert memory.is_owned_descendant(os.getpid(), os.getpid())
    assert memory.is_owned_descendant(os.getpid(), os.getppid())
    assert not memory.is_owned_descendant(os.getpid(), 987654321)


def test_supervisor_samples_verified_worker_instead_of_venv_launcher(tmp_path, monkeypatch):
    output = tmp_path/"pid.json"; samples=[]
    class Child:
        pid=111
        returncode=None
        def __init__(self,*a,**kw): memory.write_json(output,dict(pid=222,complete=True,status="passed"))
        def poll(self): return self.returncode
    child = Child()
    def sample(pid):
        samples.append(pid)
        if pid == 222: child.returncode=0
        return fake_sample()
    monkeypatch.setattr(memory, "memory_sample", sample)
    monkeypatch.setattr(memory, "is_owned_descendant", lambda pid, parent: (pid,parent)==(222,111))
    monkeypatch.setattr(memory.subprocess, "Popen", lambda *a,**kw: child)
    # Child() above prepared the handshake; supervise requires a fresh prefix.
    output.unlink()
    def spawn(*a,**kw):
        memory.write_json(output,dict(pid=222,complete=True,status="passed"));return child
    monkeypatch.setattr(memory.subprocess, "Popen", spawn)
    assert memory.supervise(SimpleNamespace(output=str(output),max_seconds=60),["unused"])==0
    assert samples==[os.getpid(),222]
    assert json.loads(output.read_text())["supervisor"]["worker_pid"]==222


def test_windows_cleanup_targets_only_owned_live_tree(monkeypatch):
    if os.name != "nt": pytest.skip("Windows process-tree cleanup")
    calls=[]
    monkeypatch.setattr(memory.subprocess,"run",lambda args,**kw:calls.append(args))
    child=SimpleNamespace(pid=12345,poll=lambda:None)
    memory.terminate_owned_tree(child)
