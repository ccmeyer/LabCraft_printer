...
@pytest.mark.parametrize("kind", ["design", "import", "import_layout_failure"])
def test_display_completion_holds_duplicate_and_shutdown_interlocks(qapp, real_editor, monkeypatch, kind):
    editor = real_editor
    editor.model.factors[0].options[0].targets = [0.5, 1, 5, 20]
    editor._load_factors_into_table()
    payload = import_payload(qapp, load_case("groups_import"), True) if kind != "design" else None
    ...
    if kind != "design":
        assert editor._pending_reagent_layout
    assert not service.submit(editor, OptimizationRequest({}), lambda _: None)
    service.shutdown(lambda: stopped.append(True))
    assert not stopped
    if kind == "import_layout_failure":
        def failed_layout():
            raise RuntimeError("injected layout failure")
        monkeypatch.setattr(editor, "_sync_reagent_tables_geometry", failed_layout)
    restore(*pending.pop())
    wait_for(qapp, lambda: stopped)
    assert len(outcomes) == 1 and outcomes[0][0] == (kind != "import_layout_failure")
    assert not service.busy and editor._optimization_ui is None
    assert not getattr(editor, "_pending_reagent_layout", False)
    if kind == "import_layout_failure":
        assert editor._design_optimization_dirty
        assert editor.model.has_uploaded_design() and len(editor.model._reactions_df) == 144
