...
def test_execution_preflight_real_operator_surface_is_exact_and_quiescent(
    tmp_path, case_id
):
    ...
    assert tuple(decisions) == EXECUTION_PREFLIGHT_SAFEGUARD_REQUIRED_ASSERTIONS
    assert set(decisions.values()) == {"pass"}
    assert case.operator_action_id in {
        row["action_id"]
