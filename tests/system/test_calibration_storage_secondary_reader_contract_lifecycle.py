...
def test_calibration_storage_secondary_reader_contract(qapp, tmp_path):
    report = run_registered_scenario(
        SECONDARY_READER_FUNCTIONAL_ID,
        output_root=tmp_path,
        speed_multiplier=1000.0,
        timeout_seconds=600.0,
        run_id="calibration-storage-secondary-reader-contract-success",
        seed=1,
    )
    ...
    assert decisions == {item: "pass" for item in FUNCTIONAL_ASSERTIONS}
    storage = report["metrics"]["persistence"]["values"]["calibration_storage"]
    secondary = storage["secondary_consumers"]
    assert secondary["memory_usable_run_count"] > 0
    assert secondary["tool_reader_state"] in {"canonical_only", "matching_dual"}
    assert secondary["consumer_error_count"] == 0
    assert secondary["legacy_hash_preserved"] is True
