...
def test_direct_lut_selector_uses_direct_axis_ownership_and_complete_inventory():
    ...
    emitter = diagnostics[
        diagnostics.index("bool resumeStatusAfterEmission = false"):
        diagnostics.index("auto runOne =", diagnostics.index("bool resumeStatusAfterEmission = false"))
    ]
    assert "comm->setStatusPaused(true);" in emitter
    assert "if (resumeStatusAfterEmission)" in emitter
    assert "comm->setStatusPaused(false);" in emitter
