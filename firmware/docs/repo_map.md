...
- Key functions:
  ...
  - `Stepper::home` accepts a cooperative cancellation token and returns distinct succeeded, failed, or canceled outcomes; every blocking home phase polls the token and restores home-only motion settings before exit. Internal homing moves consume `DirectMoveStartStatus` before waiting: only a started timer may enter the physical wait, a toward-limit `LimitBlocked` start receives one guard-bounded slow release/retry, and every other rejected or repeated start fails the home. TIM2 is left stopped/READY after peripheral initialization so the first X/home or coordinated-XY command owns its start. The wait boundary requires the stepper done event or a terminal direct-move snapshot; zero remaining toggles alone is not completion because the final physical edge precedes timer shutdown and terminal publication by one update. Toward seeks accept a confirmed limit stop as their expected terminal event, while release and final-backoff moves require an exact completed endpoint.
  ...
  - `CoordinatedXyPlanner` derives a shared X/Y active-edge rate and acceleration, sizes the transformed cosine ramp with a conservative velocity-envelope bound, and emits one cached ARR/DDA axis mask per active edge with exact centered-error endpoint counts. The centered DDA limits each participating axis to floor/ceil master-edge gaps and at most one edge of cross-track error. Preparation may divide and uses a bounded binary search for triangular peak rate; its per-edge cursor has no HAL, FreeRTOS, timer, GPIO, floating-point, division, or allocation dependency.
  - `CoordinatedXyExecutor` is the pure per-axis STEP-phase state machine. Each master event toggles only its selected axes and accounts every DEDGE-active transition as one legacy logical unit. Pause preserves both phases and the cached next event. Cancel, confirmed limit, and post-edge scheduling faults terminate immediately when both pins are low; otherwise they schedule one accounted cleanup falling edge for only the high axes, rebase targets to actual positions, and leave both pins low. Limit remains higher priority than cancel, which remains higher priority than pause.
  - `CoordinatedXyIsrInstrumentation` is the bounded production timing state. It retains master callbacks, active-edge events, per-phase and terminal maxima, complete IRQ/entry/deadline coverage, pending updates/streaks, entry lateness, scheduled timer ticks, duration, DWT wraps, and saturation. `CoordinatedXyPerformanceReport` gates exact master/per-axis active-edge counts, zero cleanup edges during successful moves, zero edge-spacing violations, and complete conditional-rearm evidence.
  - `Gantry::startCoordinatedXY` is the unconditional normal X/Y hardware adapter. It reserves both X/Y steppers, checks position/limit/timer constraints, reads the actual per-axis STEP phases, keeps TIM7 stopped, and drives shared active-edge DDA/LUT events from TIM2. A normal move starts only with matched low STEP outputs. Every physical edge is accounted before the timer schedule decision; service with at most 1,125 ticks remaining, pending UIF, or `CNT > ARR` rebases the next interval from that actual edge. `Stepper` retains direct-axis/homing ownership and forwards X/Y limits to the coordinated cursor.
  ...
  - Explicit selector `2078` runs the production-scaled camera/home transition: 5 kHz logical positioning, one camera-ratio round trip with 16,832 X and 60,000 Y active edges/60,000 TIM2 callbacks, then a bounded X home with 50 native pulses/101 timer entries. It emits result `2071` under `coordinated_xy_camera_transition_v4`; `fs` identifies the overall stage, while `mf`, `ab`, `am`, and `tm` report the exact coordinated failure mask, the 2,600-cycle active budget, and active/terminal handler maxima.
  - Explicit selector `2099` runs the incident shallow-angle suite under active manifest `coordinated_xy_shallow_edge_v4`: forward/reverse `(17100,4470)`, `(17100,2054)`, and `(100,19574)` vectors plus all axis-swapped forms at requested 10 and 40 kHz active-edge caps. The fixed finite-profile expectations are 10,000 Hz/ARR 8,999/start ARR 44,995 at 10 kHz; at 40 kHz they are 36,986 Hz/ARR 2,432/start ARR 12,160 for 17,100 master edges and 39,571 Hz/ARR 2,273/start ARR 11,365 for 19,574 master edges. Unknown combinations fail closed before the measured move. The suite gates 24 moves, 241,592 active edges per axis, 430,192 master callbacks, no TIM7 activity, cleanup edges, spacing violations, saturation, resets, watchdog events, or timeouts, asserted participating-driver enables, active-handler timing at or below 2,600 core cycles, and no more than 25 logical units of post-tier home drift. A terminal-cycle-only failure is recorded without suppressing later vectors; every other failure remains fail-fast.
  - The shared 2,600-core-cycle active-handler regression budget is the measured active-edge maximum of 2,134 cycles plus approximately 20% margin, rounded up. It is distinct from the 4,500-core-cycle physical interval at 40 kHz; deadline misses, at least 450 timer ticks of measured slack, the 1,125-tick rearm guard, and zero pending-at-rearm remain the scheduling-safety gates. Result `2100` reports 12 terminal samples per rate, per-tier minimum/mean/maximum and over-budget counts, plus the correlated common, shutdown, instrumentation, pre-handler, and full-IRQ breakdown for the worst terminal callback. Its separate 3,500-cycle limit provides about 20% margin over the accepted 2,910-cycle terminal maximum. Stage accounting must be exact, while pre-handler/full-IRQ values remain evidence only. Archived shallow manifests v1-v3 remain available for historical reports.
...
- High-level command execution/state machine:
  ...
  - `OrchestratorCompletionPolicy` centralizes the pure “did an interruptible command really finish?” bookkeeping used to decide when executed/retired frontiers may advance after pause-aware waits. Normal ABS_XY retirement additionally requires accepted startup, shared two-bit completion, a completed coordinated terminal reason, and exact endpoint/target agreement. On Resume, an interrupted ABS_XY command is classified before another event-bit wait: a valid terminal-completed coordinated snapshot at the exact retained endpoint retires once without requiring fresh one-shot completion bits, a valid active partial executor waits normally, and every inactive, failed, invalid, or mismatched state follows the existing motion-failure latch. Initial Pause interruption still takes precedence over completion, so a command that finishes while Pause is being honored remains unretired until this Resume classification. Limit, planner, rejection, and mismatch outcomes preserve the completed frontier, terminally advance the retired/current frontier through every already accepted command, drain queued tail work, and latch a fail-closed transport pause. While latched, normal queue entries receive the existing BUSY result; CLEAR and shutdown controls remain available. Pause and intentional clear/shutdown interruption retain their existing resumable or safely cancelable behavior.
  ...
  - Each persistent X/Y/Z/P/R home worker registers its static stack bounds with `CrashLog`. `Stepper::home` records phase transitions plus task-level checkpoints (`phase_entry`, `before_event_clear`, `before_move`, `waiting_for_move`, `after_move`, `before_limit_sample`, `after_limit_sample`, or `finishing`) and retains the last move-start status plus blocked-start recovery/failure counts in its RAM diagnostic snapshot. There is no per-step or timer-ISR instrumentation, and parallel X/Y or P/R scheduling is unchanged. A genuine home failure still follows the existing paused, unretired home-failure latch until Clear; it is not reclassified as a general gantry terminal fault in this slice.
...
- `tools/run_selftest.py`
  ...
  - Coordinated XY has three supported focused selectors in addition to the direct-axis LUT and pause/resume selectors. `--coordinated-xy-production-mres3-suite` maps to `2097` and emits `[2087,2088,2089,2090,2098,2106,2107,2105]` under active manifest `coordinated_xy_production_mres3_v7`. In addition to the existing MRES3 timing, driver, debounce, and bounded physical-limit gates, results `2106`/`2107` pause six coordinated XY moves at the 40 kHz application maximum and six direct Z moves at the 30 kHz application maximum. Each case requires a two-second STEP-low stable hold, exactly one bounded TMC ENN rearm with a 130 ms powered settle, a fresh remaining-distance replan starting at 3 kHz, endpoint completion, and no more than 25 logical units of post-run home drift. The qualification performs an unmeasured settling home before each measured reference home so drift is not biased by the MCU's arbitrary boot coordinate, and it opens bounded status windows during XY/Z motion for the host cadence gate. `--motion-pause-resume-suite` maps to focused selector `2109` and emits only those two results under `motion_pause_resume_v1`. Result `2098` retains the running/configured 1 MHz TIM5 and 15,000-16,000 us channel-3 timing gate; result `2105` retains the 3 kHz bounded optical-limit crossings. `MotionLimitDebouncePolicy.h` supplies the host-tested wrap-safe edge policy, while `MotionResumePolicy.h` supplies the shared 3 kHz resume-start, remaining-endpoint, and driver-rearm timing policy.
  - `--direct-xyz-lut-suite` selects production P3 selector `2096` under `FULL` and emits results `2091`-`2095`. It homes Z/XY, runs direct 14,000-logical-unit X/Y moves at 40 kHz, a Z move at 30 kHz, and a 2,000-unit triangular X move at 40 kHz, then homes again. One RAII-guarded status window remains open across the complete bounded row so sub-second move boundaries cannot hide cadence; while that selector-only window is active, the common self-test emitter resumes status after its mandatory per-frame pause. Aggregate result `2095` reports and gates status frames, a 125 ms maximum period, 100 ms watchdog age, alternation, and snapshot validity. `direct_xyz_lut_v1` requires exact MRES=3 native pulses, complete normalized cursor coverage, bounded X/Y active ISR cost with zero pending/saturation evidence, live host status/progress-watchdog checks, legacy-path homes, no P/R displacement, and the production driver configuration.
  ...
  - `--coordinated-xy-shallow-edge-suite` maps to selector `2099`, emits motion result `2099` and terminal-timing result `2100`, and is normalized by active manifest `coordinated_xy_shallow_edge_v4`. It is the required focused gate before loaded-rack testing of this defect fix.
...
- `firmware/hil/flash_and_test.sh`
  - Passes camera-benchmark and `--coordinated-xy-shallow-edge-suite` flags through to `run_selftest.py`.
  - Still returns non-zero on flash/selftest failure.
- `firmware/scripts/run_fw_hil_windows.ps1`
  - Adds `-CoordinatedXyShallowEdgeSuite` plus camera-benchmark parameters and pulls the resulting JSON artifacts.
...
| `0x24` | `TAG_RESET_XY_MOTION_CONTEXT` | 60 | optional retained gantry-motion failure record for coordinated XY or direct X/Y/Z: reason/status/state/flags, command sequence and uptime, start/target/end positions, requested/emitted edge counts, and completion bits |
...
The version-2 TLV is little-endian. Bytes 0-3 are version, fault kind, task ID, and active command; bytes 4-5 are 16-bit flags; bytes 6-10 are X/Y/Z/P/R phases; bytes 11-15 are X/Y/Z/P/R checkpoints; and bytes 16-19 are CONTROL, BASEPRI, PRIMASK, and FAULTMASK. The remaining 28 words are `EXC_RETURN`, active SP, MSP, PSP, matched stack low/high, R0-R12, LR, PC, xPSR, CFSR, HFSR, MMFAR, BFAR, FPCCR, and FPCAR. Flags identify semantic core-frame validity, extended FPU allocation, task-stack match, valid MMFAR/BFAR, handler mode, stack-allocation validity, saved R4-R11/FPU state, executable PC, Thumb xPSR, and valid checkpoints. Updated hosts also decode 112-byte version-1 records and apply PC/xPSR plausibility checks; unknown or malformed versions are ignored independently of the rest of the report. The maximum report with regulator plus CPU-fault context is 252 bytes. `INCLUDE_uxTaskGetStackHighWaterMark` remains disabled.

### Retained gantry motion context

`Orchestrator::executeAbsoluteXy()` and the shared direct-axis executor capture `XyMotionFaultContext` only after the completion policy classifies a move as `MotionFailure`. Direct `MOVE_X/Y/Z` and `ABS_X/Y/Z` commands complete only when the Stepper reports `Completed`, its final position equals the canonical command target, and its retained target still agrees. Busy/unavailable/invalid starts, profile faults, confirmed limit stops, resume mismatches, and endpoint mismatches therefore cannot retire as successful commands. Capture occurs before motor cancellation and copies the existing bounded executor snapshot; it does not poll, sample periodically, allocate memory, or reset the board. After capture, the orchestrator cancels motion, drains normal queued work, preserves `Last_completed`, advances `Last_retired` and `Current_command` through `Last_accepted`, clears the in-flight marker, and remains paused until a settled CLEAR. This creates an existing-status terminal signature without adding a status tag or changing the wire protocol.

At direct-move start, an already asserted limit may be escaped only by moving away from its home direction; motion toward an asserted limit is rejected. After that exception, any newly confirmed assertion uses the existing 15 ms debounce and terminally aborts the move regardless of direction. Homing retains its specialized limit lifecycle and is not classified by the Orchestrator as a general gantry fault.

The Python host recognizes the terminal signature only when transport is paused without a pause-after watermark, command depth is zero, accepted/retired/current agree beyond completed, and the first local nonterminal command is `RELATIVE_X/Y/Z`, `ABSOLUTE_X/Y/Z`, or `ABSOLUTE_XY`. It also recognizes a retained latch from the first zero-frontier paused status after HELLO. Detection terminally cancels unsent host work and requires a status-confirmed CLEAR followed by the exact `HOME_Z`, `HOME_XY`, `HOME_PR_BOTH` batch before ordinary commands are released.

`CrashLog.c` retains the wrapper in the existing linker `NOLOAD` `.noinit` SRAM pattern with magic/version/size/FNV-1a validation. Power and low-power resets clear it; a newer XY failure overwrites it. On reset-report selection, a pending valid CPU-fault context takes priority. Otherwise, XY context takes priority over a stale CPU-fault frame, while regulator context may accompany either. This keeps the largest XY report at 180 bytes and prevents CPU-fault plus XY context from exceeding the one-byte payload length. A successful reset-report transmission clears the retained XY record; a failed send preserves it for retry.

The version-1 value remains 60 little-endian bytes: eight one-byte header fields; command sequence and capture uptime; signed start, target, and end X/Y positions; then requested X/Y edges, emitted X/Y edges, and done bits. Coordinated XY retains the original interpretation and leaves the reserved header byte zero. A direct-axis record stores its command opcode in that reserved byte and uses the X position/edge slots as generic single-axis values; reason and terminal value `z_limit` are stable additions. Updated app and self-test decoders expose `command_type`, `axis`, `axis_start`, `axis_target`, and `axis_end` for those records, while ignoring missing, malformed, or unsupported versions without rejecting the base reset report. The host classifies any decoded gantry context as actionable and adds it to reset/debug-bundle evidence. Host black-box schema `host_black_box_v2` also copies the already-bounded `LogReader.message_history` only during failure snapshot creation or reset teardown; there is no additional continuous recorder.
