...
| `0x24` | `TAG_RESET_XY_MOTION_CONTEXT` | 60 | optional retained coordinated-XY failure record: reason/status/state/flags, command sequence and uptime, start/target/end positions, requested/emitted edge counts, and completion bits |
...
### Retained XY motion context

`Orchestrator::executeAbsoluteXy()` captures `XyMotionFaultContext` only after the existing completion policy classifies a move as `MotionFailure`. Capture occurs before motor cancellation and copies the existing coordinated executor snapshot; it does not poll, sample periodically, allocate memory, or reset the board. After capture, the orchestrator cancels motion, drains normal queued work, preserves `Last_completed`, advances `Last_retired` and `Current_command` through `Last_accepted`, clears the in-flight marker, and remains paused until a settled CLEAR. This creates an existing-status terminal signature without adding a status tag or changing the wire protocol. Reasons remain stable wire values for start rejection, X/Y limit abort, planner fault, endpoint mismatch, and resumed-command terminal mismatch.

The Python host recognizes that signature only when transport is paused without a pause-after watermark, command depth is zero, accepted/retired/current agree beyond completed, and the first local nonterminal command is `ABSOLUTE_XY`. It also recognizes a retained latch from the first zero-frontier paused status after HELLO. Detection terminally cancels unsent host work and requires a status-confirmed CLEAR followed by the exact `HOME_Z`, `HOME_XY`, `HOME_PR_BOTH` batch before ordinary commands are released.
...
The version-1 XY value is 60 little-endian bytes: eight one-byte header fields; command sequence and capture uptime; signed start, target, and end X/Y positions; then requested X/Y edges, emitted X/Y edges, and done bits. Updated app and self-test decoders ignore missing, malformed, or unsupported versions without rejecting the base reset report. The host classifies any decoded XY context as actionable and adds it to reset/debug-bundle evidence. Host black-box schema `host_black_box_v2` also copies the already-bounded `LogReader.message_history` only during failure snapshot creation or reset teardown; there is no additional continuous recorder.
