...
- High-level command execution/state machine:
  ...
  - `SelfTestSchedulingPolicy` owns result-frame transmit/yield counters and SAFE result formatting. `DiagnosticsRunner` defaults all result/progress emission to one `vTaskDelay(1)` after transmission, while P3 selectors `1039` and `1038` run the ordinary SAFE inventory in no-yield and cooperative modes. Ordinary SAFE and those selectors append `1044 pressure_wdg_context_safe` and `1043 selftest_scheduler_safe`; focused motion result inventories remain unchanged.
  - `PressureSensorWatchdogTelemetry` adds failure-only I2C attribution to those rows: `h` is the failed HAL receive result, `r` its elapsed milliseconds, and `x` the active/latest read-recovery wall duration. Normal successful reads add only two tick reads; recovery retains its existing 20 one-tick delays and no continuous tracing or loop-time stack scan is enabled.
...
- `tools/run_selftest.py`
  ...
  - `--coordinated-xy-single-irq-suite` selects diagnostic P3 selector `2075` under `FULL`. It shares selector `2077`'s ten-move 40 kHz geometry, bounded homes, status/watchdog activity, and results `2064`, `2072`, and `2073`, then adds `2074 coord_xy_single_irq_pulse`. Only this selector changes the coordinated adapter to one TIM2 interrupt per complete master step: the timer receives the full step period, direct BSRR writes raise and lower the selected STEP pins inside one ISR, and a wrap-safe DWT wait enforces the planner's 2 us minimum pulse without masking interrupts. Result `2074` requires 220,000 callbacks, pulse samples, and post-HAL deadline samples, no missing/missed deadlines or pending updates, a minimum 360-core-cycle high interval, and at least 500 90 MHz timer ticks of deadline slack. The final deadline hook samples TIM2 `CNT`, `ARR`, and `UIF` directly so tail-window overflow fails closed. A diagnostic scope guard restores `TwoEdge` on every exit; boot and ordinary motion remain two-edge.
