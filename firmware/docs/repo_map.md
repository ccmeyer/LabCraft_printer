...
- `tools/run_selftest.py`
  ...
  - `--coordinated-xy-mres3-20khz-suite` selects diagnostic P3 selector `2085` under `FULL` and emits `2080`-`2083` under `coordinated_xy_mres3_20khz_v1`. It is accepted only by the separate `MRES3_Diagnostic` image. That image configures the shared TMC2208 UART chain for `MRES=3` (1/32), `DEDGE=1`, and `multistep_filt=0`; halves X/Y/Z home/backoff parameters and all ten-move geometry coordinates; uses 20 kHz and 70,000 microsteps/s2; and preserves prior physical travel, speed, and acceleration. The unchanged two-edge executor expects 110,000 planner steps, 220,000 callbacks, and 219,990 nonterminal deadline samples. Result `2082` requires zero pending/late/deadline-miss observations and at least 450 timer ticks of margin. Result `2083` records four successful bounded UART configuration writes. P/R positions must remain unchanged. Diagnostic builds reject ordinary queued commands with all motors disabled and reject FULL selectors other than `2084`/`2085`/`2086` before motion; SAFE remains available. `Debug` stays MRES=2.
  ...
  - `--coordinated-xy-mres3-conditional-rearm-suite` selects diagnostic P3 selector `2086` and emits `[2080,2081,2082,2086,2083]` under `coordinated_xy_mres3_conditional_rearm_v1`. It preserves normal edge/ARR ordering and free-running timing above a 1,125-tick guard, then rebases CNT only for low margin, UIF, or CNT beyond ARR. A task-context one-shot arms one bounded 900-tick cruise-rise injection for each of the ten measured moves. Result `2086` gates 219,990 complete decisions, ten successful injected rearms, non-rearmed slack above the guard, bounded wait cycles, and zero missing/pending/saturation/timeout evidence. Intentional waits are excluded only from executor phase-cost accounting; raw IRQ and duration evidence retain them. Mode and injection state are restored/cleared on every exit, and production remains `FreeRunning`.
