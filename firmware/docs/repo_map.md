...
- `tools/run_selftest.py`
  ...
  - `--z-speed-ladder-suite` maps to diagnostic-only P3 selector `2199` and emits `[2190,2191,2192,2193,2194]` under active manifest `z_speed_ladder_v1`. It reuses the long-Z `(43000,13000)` anchor and `Z=100..80000` endpoints for three out/return/home repetitions at 30/40/50/60 kHz logical rates, with operator gates before the higher tiers. `Stepper::armZSpeedDiagnosticInstrumentation()` enables earliest TIM10 shared-vector entry capture and post-HAL exit/deadline accounting only for measured Z legs; the hook stays dormant for ordinary Z, homes, and selector `2096`. `ZAxisSpeedLadderReport` owns the pure aggregation, exact pulse/callback coverage, wrap-safe DWT accounting (at most one wrap per measured leg), ISR/deadline/status/home/limit gates, and compact metrics. Summary `2194` also requires a valid run-local pressure telemetry snapshot with no I2C selection/read failure, bus recovery, or saturation.
