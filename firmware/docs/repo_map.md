...
- `tools/run_selftest.py`
  ...
  - `--coordinated-xy-40khz-suite` selects P3 selector `2077` under `FULL` and emits existing row result `2064` plus correlated IRQ-path evidence `2072` under `coordinated_xy_40khz_v1`. It reuses the exact ten-move 40 kHz geometry row and bounded post-row X/Y homes, skipping tiers 5-30 kHz, the focused X-direction workload, and all later raster/camera/pressure cases. The generated TIM2 ISR is touched only inside its USER CODE blocks: it captures DWT at the earliest user hook and again after `HAL_TIM_IRQHandler()` returns. Gantry correlates those values with its existing inner phase/terminal timing and pending-update sample, including separate non-terminal and terminal full-path maxima. It requires one `motion_clear_envelope_v1` confirmation and does not actuate pressure.
