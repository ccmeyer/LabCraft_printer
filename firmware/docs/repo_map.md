...
- `tools/run_selftest.py`
  ...
  - `--z-speed-ladder-suite` maps to diagnostic-only P3 selector `2199` and emits `[2190,2191,2192,2194]` under active manifest `z_speed_ladder_v2`. It reuses the long-Z `(43000,13000)` anchor and `Z=100..80000` endpoints for three out/return/home repetitions at 30/40/50 kHz logical rates, with operator gates before the higher tiers. `Stepper::armZSpeedDiagnosticInstrumentation()` enables earliest TIM10 shared-vector entry capture and post-HAL exit/deadline accounting only for measured Z legs; the hook stays dormant for ordinary Z, homes, and selector `2096`. `ZAxisSpeedLadderReport` owns pure aggregation, exact pulse/callback coverage, wrap-safe DWT accounting (at most one wrap per measured leg), a 2,250-cycle stepping-body gate, a separate 2,550-cycle complete-vector gate, at least 900 core cycles of measured deadline slack, and status/home/limit gates. Summary `2194` also requires a valid run-local pressure telemetry snapshot with no I2C selection/read failure, bus recovery, or saturation. Manifest v1 and catalog row `2193` remain archived for historical report normalization.
