...
`Orchestrator::executeAbsoluteXy()` captures `XyMotionFaultContext` only after the existing completion policy classifies a move as `MotionFailure`. Capture occurs before motor cancellation and copies the existing coordinated executor snapshot; it does not poll, sample periodically, allocate memory, reset the board, retire the command, or alter the motion-stop behavior. Reasons are stable wire values for start rejection, X/Y limit abort, planner fault, endpoint mismatch, and resumed-command terminal mismatch.
