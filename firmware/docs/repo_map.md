...
- Key functions:
  ...
  - `CoordinatedXyIsrInstrumentation` is the compile-gated Milestone 6 timing state. The per-edge recorder uses bounded integer operations to separate acceleration, cruise, deceleration, and terminal-cleanup cycles while counting pulses, pending updates/streaks, scheduled timer ticks, DWT wraps, and saturation. `CoordinatedXyPerformanceReport` validates completed task-context snapshots and emits compact qualification metrics.
  - `Gantry::startCoordinatedXY` is the gated hardware adapter. It reserves both X/Y steppers, checks position/limit/timer constraints, keeps TIM7 stopped, and drives shared DDA/LUT events from TIM2. `Stepper` owns narrow Gantry-only reservation, pin, direction, position, target, and EXTI limit-forwarding hooks. The TIM2 adapter samples both raw physical-limit inputs before each potential rising edge as a redundant fallback to EXTI forwarding.
  - The Milestone 5 candidate sets `LC_COORDINATED_XY_NORMAL_ROUTE_ENABLE=1`, so `Gantry::moveTo` and XY-only `moveBy` use the shared executor. A build definition of `LC_COORDINATED_XY_NORMAL_ROUTE_ENABLE=0` retains the independent X/TIM2 and Y/TIM7 A/B path. Mixed XY+Z `moveBy` requests are rejected before any hardware or completion state changes; Z-only, direct-axis, homing, and pressure motion stay legacy-routed.
  - Explicit selector `2078` runs only the Milestone 6 camera/home transition diagnostic: 5 kHz positioning, one 40 kHz camera-ratio round trip, then immediate bounded legacy X home with enable, STEP, ownership, timer, and limit evidence. It does not alter the normal route or pressure paths.
...
- `build_firmware_headless.ps1` imports the CubeIDE project and performs `-cleanBuild "$ProjName/$Cfg"`; then copies the newest `.bin` to `firmware/artifacts/`, skipping the copy if the build output is already the artifact path. `-Config MRES3_Diagnostic` requires a distinct `-ArtifactFileName`, preventing it from overwriting the production `LabCraft_firmware.bin`.
...
- `tools/run_selftest.py`
  ...
  - `--coordinated-xy-executor-suite` selects P3 selector `2049` under `FULL`. It first disables XY and verifies manual press/release of both physical limit inputs, then homes X/Y sequentially at reduced rate, anchors at `(5000,5000)`, and runs the gated TIM2-master executor at 3 kHz for results `2040`-`2046`. `coordinated_xy_executor_v1` evaluates pulse counts, edge counts, pause/cancel/limit behavior, pin idle state, ISR cycles, recovery, and teardown-home drift.
  - `--normal-xy-route-suite` selects P3 selector `2059` under `FULL` and emits results `2050`-`2057`. `normal_xy_route_v1` requires the `coordinated_xy_physical_limit_v1` fixture, manually verifies both switches with motors disabled, homes Z then X/Y at reduced rates, caps normal XY at 3 kHz, and checks normal-route completion/status/control behavior, bounded physical X/Y limit crossings, and unchanged direct X/Y/Z ownership. Pressure qualification remains a separate `pressure_regulator_v1` run with `pressure_closed_loop_v1` installed.
  - `--coordinated-xy-performance-suite` selects P3 selector `2069` under `FULL` and emits results `2060`-`2068` plus focused investigation result `2070`. `coordinated_xy_performance_v1` requires exactly one combined `pressure_closed_loop_v1` fixture/envelope confirmation. The manual switch preflight and low-rate homing regression passed after the bounded-home change, and no subsequent code affects switches or homing. It fails closed through a 5-40 kHz geometry ladder, direction-isolated X checks at 30/35/40 kHz with normal and reduced acceleration, exact Milestone 1 vectors, a Z-up 16x24 raster, repeated camera/home-ratio motion, and concurrent P/R regulation while recording bounded-home evidence, phase timing, pending updates, duration error, status cadence, watchdog state, home drift, and exact pulse/callback totals.
  - `--coordinated-xy-40khz-suite` selects P3 selector `2077` under `FULL` and emits existing row result `2064`, correlated IRQ-path evidence `2072`, and entry-lateness evidence `2073` under `coordinated_xy_40khz_v1`. It reuses the exact ten-move 40 kHz geometry row and bounded post-row X/Y homes, skipping tiers 5-30 kHz, the focused X-direction workload, and all later raster/camera/pressure cases. The generated TIM2 ISR is touched only inside its USER CODE blocks: it captures DWT, TIM2 `CNT`, and TIM2 `ARR` at the earliest user hook and DWT again after `HAL_TIM_IRQHandler()` returns. Gantry performs bounded integer aggregation only after its established pending-update observation. Result `2073` exposes complete/missing coverage, counter-at-entry maximum/mean, pending-correlated maximum, a diagnostic 128-tick count, inter-entry schedule overrun, synchronization mode, saturation, and the first failed leg's terminal reason plus coordinated/raw limit counts. It requires one `motion_clear_envelope_v1` confirmation and does not actuate pressure.
  - `--coordinated-xy-status-sync-suite` selects diagnostic P3 selector `2076` under `FULL` and reuses selector `2077`'s exact result IDs, ten-move geometry, instrumentation, and bounded homes. It changes only status-metric reset/update/snapshot synchronization from the production critical section to a dedicated statically allocated task mutex with a bounded 5 ms wait, reports `sm=1` plus lock failures `lf`, and restores critical-section mode on every exit. `coordinated_xy_status_sync_v1` applies the strict `lc=0`, `cm<128`, `dm<256`, zero-pending, complete-coverage, zero-failure-telemetry, cadence, watchdog, and home gates. Normal boot and operation remain `sm=0`.
  - `--coordinated-xy-single-irq-suite` retains P3 selector `2075` only as a fail-closed compatibility route. It emits `gate=single_irq_superseded` before any fixture prompt or motion; the two-edge executor remains selected.
  - `--direct-xyz-lut-suite` selects production P3 selector `2096` under `FULL` and emits results `2091`-`2095`. It homes Z/XY, runs direct 14,000-logical-unit X/Y/Z moves plus a 2,000-unit triangular X move at 40 kHz logical rate, then homes again. `direct_xyz_lut_v1` requires exact MRES=3 native pulses, complete normalized cursor coverage, bounded X/Y active ISR cost with zero pending/saturation evidence, live status/watchdog cadence, legacy-path homes, no P/R displacement, and the production driver configuration.
  - `--coordinated-xy-mres3-20khz-suite` selects diagnostic P3 selector `2085` under `FULL` and emits `2080`-`2083` under `coordinated_xy_mres3_20khz_v2` (v1 is retained for historical reports). It is accepted only by the separate `MRES3_Diagnostic` image. That image configures the shared TMC2208 UART chain for `MRES=3` (1/32), `DEDGE=1`, and `multistep_filt=0`. The selector submits the original logical geometry, 40 kHz rate, and 140,000 logical-units/s2; `MotionUnitScale` converts them at the motor boundary to 20 kHz, 70,000 native cycles/s2, 110,000 planner cycles, 220,000 callbacks, and 219,990 nonterminal deadline samples while preserving physical motion. Result `2083` records four successful bounded UART configuration writes. For complete, internally consistent measured moves, selector `2085` records timing-only qualification failures and continues the row; `2080.qf/qm` retain strict failures and `2082.hm` identifies the first hard stop. P/R positions must remain unchanged. Diagnostic builds reject ordinary queued commands with all motors disabled and reject FULL selectors other than `2084`/`2085`/`2086` before motion; SAFE remains available. Diagnostic boot stays `FreeRunning`.
  - `--coordinated-xy-mres3-rearm-suite` selects diagnostic P3 selector `2084` in the same `MRES3_Diagnostic` image and uses the identical logical geometry, homes, speed, acceleration, result inventory, and TMC settings as `2085`. An RAII guard temporarily selects `RearmFromActualEdge` and restores the diagnostic boot/default `FreeRunning` mode on every exit. After each nonterminal STEP rise/fall, Gantry atomically stops TIM2, snapshots any pending UIF, resets CNT, clears UIF/NVIC pending state, and restarts TIM2. Result `2082` adds `rm`, `rc`, `rp`, and `rd`; `coordinated_xy_mres3_rearm_v1` requires all 219,990 rearms, no pending-at-rearm event, complete zero-miss deadline evidence, and at least 450 timer ticks of slack while retaining entry lateness as diagnostic evidence.
  - `--coordinated-xy-mres3-conditional-rearm-suite` selects diagnostic P3 selector `2086` and emits `[2080,2081,2082,2086,2083]` under `coordinated_xy_mres3_conditional_rearm_v3` (v1/v2 are retained for historical reports). Modes 0/1 dispatch to a nonconditional TIM2 specialization; only mode 2 compiles timer sampling, the one-shot 900-tick peak-rate injection, conditional decisions, and intentional-wait subtraction into its ISR body. The injection uses the first cruise rise when a plateau exists and otherwise the first deceleration rise at the zero-cruise peak. Mode 2 preserves normal edge/ARR ordering and free-running timing above a 1,125-tick guard, then rebases CNT only for low margin, UIF, or CNT beyond ARR. Result `2086` gates 219,990 complete decisions, ten successful injected rearms, non-rearmed slack above the guard, bounded wait cycles, and zero missing/pending/saturation/timeout evidence. Completed timing-only failures may be retained through the complete row but remain strict result failures; integrity, safety, incomplete-evidence, saturation, reset/watchdog, and communication failures stop immediately. Intentional waits are excluded only from executor phase-cost accounting; raw IRQ and duration evidence retain them. Mode and injection state are restored/cleared on every exit. Selector `2084` is unchanged.
  - `--coordinated-xy-production-mres3-suite` selects P3 selector `2097` in the production `Debug` image and emits `[2087,2088,2089,2090]` under `coordinated_xy_production_mres3_v1`. Production programs MRES=3/DEDGE=1/multistep_filt=0, preserves legacy logical units through `MotionUnitScale`, and boots coordinated XY in `ConditionalLateRearm`. Synthetic late-edge injection and intentional-wait code are compiled out of production, so production evidence requires all injection fields to remain zero while allowing either zero natural rearms or internally consistent natural-rearm evidence. The selector strictly gates native counts, complete decisions/IRQ coverage, active-callback cost, physical-edge deadline margin, conditional-rearm consistency, bounded homes, cadence, watchdog/reset state, and driver configuration. Terminal cleanup cost remains reported as non-blocking candidate telemetry because that callback occurs after the final STEP edge.
  - `--coordinated-xy-x-direction-suite` selects P3 selector `2079` under `FULL` and emits only result `2070` under `coordinated_xy_x_direction_v1`. It shares the M6 fixture and bounded-homing evidence path but, after the accepted manual switch preflight, requires only the fixture/envelope confirmation when intervening code does not affect switch or homing behavior. It terminates after the eight direction-isolated X legs and cannot fall through into the full M6 rows.
