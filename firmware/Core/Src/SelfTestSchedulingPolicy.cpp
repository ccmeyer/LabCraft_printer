                                  ...
                                  size_t metricsLen)
{
  ...
  const bool pass = knownMode && delayCountValid && pressure.valid != 0u &&
                    stackEvidenceComplete && !saturated;
  if (metrics != nullptr && metricsLen > 0u) {
    std::snprintf(metrics,
                  metricsLen,
                  "sm=%u;rf=%lu;yc=%lu;txm=%lu;txt=%lu;pg=%lu;pa=%lu;ph=%u;pha=%lu;la=%lu;se=%lu;re=%lu;bc=%lu;hw=%lu;sf=%u",
                  ...
                  saturated || pressure.valid == 0u || !stackEvidenceComplete ? 1u : 0u);
  }
  return pass;
}
                                              ...
                                              size_t metricsLen)
{
  ...
  const bool pass = (!contextExpected || contextValid) && stackEvidenceComplete &&
                    (!contextValid || context.saturated == 0u);
  if (metrics != nullptr && metricsLen > 0u) {
    std::snprintf(metrics,
                  metricsLen,
                  "v=%u;ex=%u;pa=%lu;ph=%u;pha=%lu;la=%lu;pg=%lu;se=%lu;re=%lu;bc=%lu;lp=%lu;hw=%lu;tk=%lu;sf=%u",
                  contextValid ? 1u : 0u,
                  ...
                  contextValid &&
                      (context.saturated != 0u || !stackEvidenceComplete) ? 1u : 0u);
  }
  return pass;
}
