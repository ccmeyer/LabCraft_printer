...
TEST_CATALOG: dict[int, TestCatalogEntry] = {
    ...
    2071: TestCatalogEntry(2071, "Coordinated XY camera/home transition", "Cold 40 kHz camera-ratio round trip followed immediately by bounded legacy X homing, with the exact coordinated failure mask, active/terminal handler timing, enable, timer, STEP, and limit evidence."),
    ...
    2087: TestCatalogEntry(2087, "Production MRES3 coordinated motion", "Production MRES=3 ten-move row using exact DEDGE active-transition totals, uniform edge distribution, and the evidence-based active-handler regression budget."),
    2088: TestCatalogEntry(2088, "Production MRES3 IRQ path", "One-callback-per-active-edge TIM2 IRQ coverage for production MRES=3 coordinated motion."),
    ...
    2098: TestCatalogEntry(2098, "Coordinated XY motion-limit debounce", "Edge-aware EXTI plus 1 MHz TIM5 X/Y limit confirmation, transient rejection, timing-health, and clean-motion evidence."),
    2099: TestCatalogEntry(2099, "Coordinated XY shallow-edge distribution", "Incident shallow-angle vectors and swapped forms at requested 10 and 40 kHz active-edge caps with exact finite-profile rates, bounded active-handler timing, spacing, cleanup, endpoint, and home-drift evidence."),
    2100: TestCatalogEntry(2100, "Coordinated XY terminal timing", "Per-rate terminal callback distributions and correlated worst-callback stage timing under the shared 3,500-cycle post-motion regression gate."),
    2105: TestCatalogEntry(2105, "Coordinated XY physical limit crossing", "Bounded 3 kHz physical X/Y optical-limit crossings with correct-axis terminal, forced-low STEP outputs, post-edge travel, and release/home recovery evidence."),
    2106: TestCatalogEntry(2106, "Coordinated XY pause/resume fresh replan", "Six 40 kHz coordinated moves paused across early, middle, and late profile phases; STEP-low two-second holds; one bounded TMC ENN rearm and powered settle per resume; 3 kHz fresh replans of the remaining endpoint; and post-run X/Y home drift."),
    2107: TestCatalogEntry(2107, "Direct Z pause/resume fresh replan", "Six 30 kHz direct Z moves with STEP-low two-second holds, one bounded TMC ENN rearm and powered settle per resume, 3 kHz fresh replans of the remaining endpoint, and post-run Z home drift."),
