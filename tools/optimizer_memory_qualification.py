...
def _windows_api():
    ...
    return kernel, psapi, Counters, Status, ProcessEntry


def is_owned_descendant(pid, ancestor):
    if pid == ancestor:
        return True
    parents = {}
    if sys.platform == "win32":
        kernel, _, _, _, Entry = _windows_api()
...
def terminate_owned_tree(process, force=False):
    # Windows venv launchers may own the actual Python process. Terminating only
    # that launcher would orphan its worker. The root is our live Popen child.
...
def memory_sample(pid):
    ...
    if sys.platform == "win32":
        kernel, psapi, Counters, Status, _ = _windows_api()
    ...
    with output.with_suffix(".worker.log").open("w", encoding="utf-8") as log, output.with_suffix(".samples.jsonl").open("w", encoding="utf-8") as samples:
        ...
        try:
            ...
            while process.poll() is None:
                if worker_pid is None and output.exists():
                    reported = json.loads(output.read_text()).get("pid")
                    if reported is not None:
                        if not isinstance(reported, int) or not is_owned_descendant(reported, process.pid):
                            raise RuntimeError("Reported Qt worker PID is outside the owned process tree")
                        worker_pid = reported
                try:
                    sample = memory_sample(worker_pid or process.pid)
                except (OSError, KeyError):
                    if process.poll() is not None:
                        break
                    # A Windows venv launcher can outlive its finished worker
                    # briefly. Its final checkpoint proves orderly Qt shutdown.
                    if output.exists() and json.loads(output.read_text()).get("worker_stopped"):
                ...
                sample["pid"] = worker_pid or process.pid
                sample["worker_identified"] = worker_pid is not None
            ...
            if process.poll() is None:
                ...
                try:
                    ...
                    try: process.wait(timeout=5)
                        ...
                        terminate_owned_tree(process, force=True)
                        cleanup.append("killed owned process after termination grace")
                        process.wait(timeout=5)
            for sig, handler in handlers.items(): signal.signal(sig, handler)
            record = dict(pid=process.pid, worker_pid=worker_pid, returncode=process.poll(), cleanup=cleanup, stop_reason=reason,
