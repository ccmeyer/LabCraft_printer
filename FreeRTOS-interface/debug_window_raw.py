...
def format_bytes(src: str, data: bytes) -> str:
    # one-line: hex + printable ascii
    hexpart = " ".join(f"{b:02X}" for b in data)
    asciipart = "".join(chr(b) if 32 <= b < 127 else "." for b in data)
    return f"[{src}] {hexpart}    |{asciipart}|"

class FrameParser:
    """
    Incremental parser for frames:
...
class RawSerialReader(QThread):
    rawReceived = Signal(str)       # formatted raw line
    status_received = Signal(dict)  # parsed TLV status dict (optional)
    error = Signal(str)

    def __init__(self, port: str, baud: int, name: str, parent=None):
    ...
    def run(self):
        try:
            ...
            self.error.emit(f"{self.name}: open failed: {e}")
            return
        ...
        try:
            while not self.isInterruptionRequested():
                ...
                # Always show raw bytes
                self.rawReceived.emit(format_bytes(self.name, data))

                # Try to parse frames
                self.parser.push(data)
                for frame in self.parser.pop_frames():
                    # frame = [AA][len][payload...][crcLo][crcHi]
                    length = frame[1]
                    payload = frame[2:2+length]
                    rec_crc = frame[2+length] | (frame[2+length+1] << 8)
                    calc_crc = crc16_x25(payload)
                    if rec_crc != calc_crc:
                        self.rawReceived.emit(f"[{self.name}] ! CRC ERROR rec=0x{rec_crc:04X} calc=0x{calc_crc:04X}")
                        continue

                    # if it’s a status frame, parse TLV
                    if payload and payload[0] == CMD_STATUS:
                        data_dict = parse_tlv_payload(payload[1:])
                        if data_dict:
                            self.status_received.emit(data_dict)

        except Exception as e:
            self.error.emit(f"{self.name}: reader error: {e}")
        finally:
            try:
...
class MainWindow(QWidget):
    def __init__(self, port, baud):
        ...
        # Start RAW reader for COMM (same port you send commands on)
        self.comm_reader = RawSerialReader(port=port, baud=baud, name="COMM")
        self.comm_reader.rawReceived.connect(self.append_cmd_log)   # show raw in cmd_log
        self.comm_reader.status_received.connect(self.update_status)
        self.comm_reader.error.connect(self.on_error)
        self.comm_reader.start()

        # Start RAW reader for LOGGER (USB serial)
        self.log_reader = RawSerialReader(port="/dev/ttyUSB0", baud=baud, name="LOG")
        self.log_reader.rawReceived.connect(self.append_line)       # show raw in logView
        self.log_reader.error.connect(self.on_error)
    ...
    def closeEvent(self, event):
        for t in [getattr(self, "comm_reader", None), getattr(self, "log_reader", None)]:
            if t:
