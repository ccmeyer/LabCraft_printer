...
class SpeedProfilesTab(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        monitor_v.addWidget(self.logs_group, 1)
    ...
    def _restore_after_publication(self, result, manager):
        try:
            if not isValid(self.owner):
                return
            try:
                if getattr(self.owner, "_pending_reagent_layout", False):
                    self.owner._pending_reagent_layout = False
                    self.owner._sync_reagent_tables_geometry()
            except Exception as exc:
                # The model is already committed. Keep it, report the display
                # failure and release the controls/job so the user can retry.
                result = (False, {"reason": str(exc), "status": "failed"})
                self.owner._mark_design_optimization_dirty()
                self.status(str(exc))
            self.dialog.close()
...
class ExperimentDesignDialog(QDialog):
    ...
    def _load_factors_into_table(self, *, defer_layout=False):
        ...
        if defer_layout:
            self._pending_reagent_layout = True
        else:
            self._sync_reagent_tables_geometry()
    ...
    def _apply_uploaded_design_payload(self, payload: Mapping[str, Any]):
        ...
        def finished(outcome):
            ...
            self._load_factors_into_table(defer_layout=getattr(self, "_optimization_ui", None) is not None)
            self._update_unique_conditions_button_label()
            self._mark_draft_dirty()
