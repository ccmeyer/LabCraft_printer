...
| 2026-08-24 | Exact-tag legacy-cohort rehearsal tooling implemented | Added a standalone-clone public-updater lane, immutable real-backup binding, separate cancellation/activation roots, bootstrap-only visible review, exact migration comparison, second-checkout reopen, protected-invariance seals, and two-cohort aggregation | Initial rc.1 Prepare failed closed on an incorrect persisted `production_ready` assumption; corrected firmware-state v1 parsing derives readiness from `role`; failed root retained; 228 focused Windows tests passed | Publish and Pi-qualify the corrective harness, then prepare fresh rc.1/rc.6 runs and seal both before fleet pilots |
