...
| 2026-08-24 | Exact-tag legacy-cohort rehearsal tooling implemented | Added a standalone-clone public-updater lane, immutable real-backup binding, separate cancellation/activation roots, bootstrap-only visible review, exact migration comparison, second-checkout reopen, protected-invariance seals, and two-cohort aggregation | 224 focused Windows tests passed; commit and push the exact harness, complete Pi dry-run/focused qualification, then run and seal one real rc.6 and one real rc.1 rehearsal before fleet pilots |
