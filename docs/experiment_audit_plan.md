...
| Phase 6: Audit Timeline Reader | Complete | Added read-only `ExperimentAuditReader` timeline parser and stable row/table data in `FreeRTOS-interface/ExperimentAuditReader.py`; added focused reader coverage in `tests/test_experiment_audit_reader.py`. | `.\env\Scripts\python.exe -m pytest -q tests/test_experiment_audit_reader.py` -> 12 passed. `.\env\Scripts\python.exe -m pytest -q tests/test_experiment_audit_log.py tests/test_experiment_audit_integration.py` -> 13 passed. `.\env\Scripts\python.exe -m pytest -q tests/test_controller_experiment_audit.py tests/test_calibration_experiment_audit.py` -> 19 passed. |
| Phase 7: Audit Timeline Window | Complete | Added the read-only Qt audit timeline table/window and main-window button/shortcut in `FreeRTOS-interface/View.py`; added focused window/model coverage in `tests/test_experiment_audit_window.py`. | `.\env\Scripts\python.exe -m pytest -q tests/test_experiment_audit_window.py tests/test_experiment_audit_reader.py` -> 20 passed. `.\env\Scripts\python.exe -m pytest -q tests/test_view_array_controls.py tests/test_view_window_icon_contract.py` -> 16 passed. `.\env\Scripts\python.exe -m pytest -q tests/test_experiment_audit_log.py tests/test_experiment_audit_integration.py` -> 13 passed. |
| Phase 8: Operator Notes and Export | Complete | Added append-only operator notes and Markdown export controls in `FreeRTOS-interface/View.py`; added pure Markdown rendering in `FreeRTOS-interface/ExperimentAuditReader.py`; added focused note/export coverage in `tests/test_experiment_audit_notes.py`. | `.\env\Scripts\python.exe -m pytest -q tests/test_experiment_audit_notes.py tests/test_experiment_audit_window.py tests/test_experiment_audit_reader.py` -> 31 passed. `.\env\Scripts\python.exe -m pytest -q tests/test_experiment_audit_log.py tests/test_experiment_audit_integration.py` -> 13 passed. `.\env\Scripts\python.exe -m pytest -q tests/test_controller_experiment_audit.py tests/test_calibration_experiment_audit.py` -> 19 passed. |
| Post-MVP: Audit Usability Polish | Complete | Added derived stock-solution display, compact timeline tooltips, reset audit events, Markdown stock column, and calibration audit filtering for volume-producing calibration processes only. Reset events are warning-level; calibration session rows and non-volume calibration process rows are omitted from the high-level audit. | `.\env\Scripts\python.exe -m pytest -q tests/test_experiment_audit_reader.py tests/test_experiment_audit_window.py tests/test_experiment_audit_notes.py` -> 34 passed. `.\env\Scripts\python.exe -m pytest -q tests/test_controller_experiment_audit.py tests/test_controller_print_guards.py` -> 55 passed. `.\env\Scripts\python.exe -m pytest -q tests/test_calibration_experiment_audit.py tests/test_calibration_memory_integration.py` -> 26 passed. `.\env\Scripts\python.exe -m pytest -q` -> 1917 passed, 1 skipped. |
| Post-MVP: Calibration Stock Identity | Complete | Added concentration-aware calibration stock identity snapshots to calibration audit events, `calibration.json` run/step metadata, process recorder metadata, and flat calibration measurement rows; updated audit reader stock display precedence for structured `stock_identity` payloads. | `.\env\Scripts\python.exe -m pytest -q tests/test_calibration_experiment_audit.py tests/test_calibration_memory_integration.py tests/test_experiment_audit_reader.py` -> 46 passed. `.\env\Scripts\python.exe -m pytest -q tests/test_experiment_audit_window.py tests/test_experiment_audit_notes.py` -> 20 passed. `.\env\Scripts\python.exe -m pytest -q` -> 1927 passed, 1 skipped. |
