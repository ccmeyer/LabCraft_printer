...
- Firmware rows `2473` and `2474` compare print and refuel valves at `2 psi=3386`, across matched grouped `1500`, `3000`, and `4500 us` pulse widths with one unmeasured conditioning pulse before each channel/width group. The first measured pulse after each width change is kept in the artifacts but excluded from steady fixed-width settled-drop aggregates.
...
- Row `2475` derives print/refuel channel balance from the isolated 2 psi steady settled-drop rows without additional valve actuation.
...
- Gap sweep settled-drop magnitudes remain informational; execution checks still require startup homing, pressure readiness, fresh samples, trace sample/event capture, and zero settled-drop rejects. Missing latency/ring detections are retained as diagnostic context rather than pass/fail failures.
