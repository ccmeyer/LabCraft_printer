...
- Normal pressure-regulator setpoint changes now slew the control target at `1.5 psi/sec` (`1311 raw/sec`) and cap commanded motor speed at `16000 Hz` only while a setpoint transition is active. Once the ramp reaches the requested target, ordinary pressure maintenance and dispense recovery use the existing behavior.
