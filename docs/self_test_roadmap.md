...
| Milestone 8I: Gripper Seal Stress and Motion Suite | In progress | Active operator-gated `gripper_seal_stress_v2` adds a production-wire profile/dispense gate before firmware rows `2510`-`2513`; archived v1 remains available only for historical normalization and neither suite is added to default FULL |
...
- Firmware rows `2012`-`2016` cover reverse long XY travel, diagonal XY travel, a 16 x 24 plate raster from `(43000,13000)` toward `(33000,30000)`, Z long travel to `80000` from the raster-start XY anchor, and homing from already-triggered X/Y/Z limit starts.
- Row `2014` moves to the plate start with Z homed/up, waits for an operator evaporation-plate confirmation using existing `CMD_RESUME`, lowers Z to `91500`, rasters, returns XY to the plate start, homes Z, then performs the existing XY return/home behavior.
...
- Preserve `gripper_seal_stress_v1` as an archived historical manifest and use active `gripper_seal_stress_v2` as the operator-gated FULL manifest requiring fixture `dummy_blocked_head_motion_v1`.
- Before selector `2599`, v2 runs a headless production-wire check through the existing queue protocol: set a safe 1 psi print target, start regulation, prove fresh active/ready pressure status, explicitly close, `ENABLE_PRINT_PROFILE(p1=1)`, run a fast pre-expiry print-only dispense, run a post-expiry pair proving a `3000..7000 ms` retirement gap, disable, and run a 31-second `p1=0` negative window with two fast dispenses. It records exact ACK/sequence/retirement and pressure-readiness evidence in `production_path.json`; disable and print-pressure deregulation are attempted on every exit with `CLEAR` fallback, and this layer never releases or powers off an unsupported dummy head.
...
- `2510` measures static dummy-head seal response at 1, 2, and 3 psi with gripper refresh disabled, using one unmeasured conditioning pulse per pressure before five measured decimated traces per channel. `2511` enables the production deferred policy at `30000 ms`, verifies expiry remains pending and non-actuating through repeated 3 psi direct diagnostic challenges, uses an actual `Printer::enqueueWithTimeout()` completion to claim exactly one refresh, verifies rearm after pump-off, and measures at least `3000 ms` before an immediately queued second dispense completes. `2512` enables the same deferred policy for the motion window, homes Z as a clearance preflight and then homes X/Y before any XY motion, moves to the plate start with Z homed/up, waits for an operator evaporation-plate confirmation using existing `CMD_RESUME`, lowers Z to `91500`, runs the 384-well endpoint-interpolated plate raster while launching 3 psi regulator-quiet pulse-window traces at `10000 ms` intervals, and verifies that pending expiry is not consumed and the gripper pump is not actuated by motion or direct diagnostic pulses. It then returns XY to the plate start, homes Z, and parks before reporting. `2513` compares pre/post-raster 3 psi static seal traces; it is skipped with `gate=z_clearance_home`, `gate=evap_plate_setup`, or `gate=evap_plate_teardown` if row `2512` cannot safely complete the raster. Every diagnostic success, failure, timeout, and abort path disables deferred refresh.
...
- Keep `gripper_seal_v1` as the quick local baseline and keep `gripper_seal_stress_v2` out of `factory_acceptance_v3` until enough dummy-head motion-stress data exists to set acceptance thresholds.

Campaign runner backend slice:

- `machine_full_qualification_v1` queues the production rigorous suites: `motion_envelope_v1`, `pressure_regulator_v1`, `valve_characterization_v1`, and active `gripper_seal_stress_v2`.
