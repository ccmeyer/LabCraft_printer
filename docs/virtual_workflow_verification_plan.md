...
The execution read-elimination remediation is verified. Preserve its
one-identity-guard-per-write invariant, three resume writes plus one progress
write per completion, fail-closed external-change behavior, and terminal full
validation. The remaining measured persistence cost is growing JSON
serialization and the four intentionally retained durable writes. Slice 8 is
the next permitted body of work but remains `deferred` until it has a separate
reviewed plan; do not begin it as part of this remediation.
