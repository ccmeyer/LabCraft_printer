...
| 2026-07-24 | Performance remediation | compact authoritative progress `not_started` -> `in_progress` | Starting commit `b1c027b76fa7c9908faa9af59dec92c951568d10` had a clean tracked worktree; the inaccessible pre-existing pytest temporary directories remain untracked and untouched. Call path: Controller completion -> validated pending intent/live count -> copy one compact stock array -> deterministic complete-snapshot serialization -> unchanged flush/fsync/atomic replace -> identity acceptance -> intent retirement. Scope is limited to a central progress codec, authoritative Model/load integration, offline downgrade tool, compatible benchmark/scenario diagnostics, focused tests, README, report schema, and this record. Controller commands, UI, simulator, production machine, firmware, protocol, motion, pressure, and all four durability boundaries remain unchanged. The captured schema-v1 384x10 snapshot was 1,064,483 bytes and projected 4.088 GB of full-run serialization. New authoritative and safe zero-progress executions adopt schema v2; positive or checkpointed v1 executions remain v1, and legacy/non-authoritative paths retain v1. Risks are plan/array mismatch, unsafe silent conversion, cache advancement before durability, and rollback incompatibility; mitigations are strict codec validation, conservative adoption, post-success advancement, failure injection, semantic fingerprints, and the validated offline v2-to-v1 converter. Focused implementation validation currently passes 92 tests, including reduced real-UI multi-stock completion with exact durability/read invariants. Full 384x10, Pi, full-suite, artifact inspection, and final closure evidence remain pending. Slice 8 remains deferred. |
| 2026-07-24 | Performance remediation | compact authoritative progress implementation verified; stress responsiveness blocker retained | Implementation commit `70af7f4e` and documentation commit `dca2fc445730` add strict v1/v2 decoding, deterministic compact v2 encoding, one-array copy-on-write updates, conservative adoption, raw/expanded cache advancement only after durability, the offline validated v2-to-v1 converter, format/size/cumulative-byte diagnostics, fixture integration, and compatibility/failure tests. Focused validation passed 92 tests in 20.33 s and the broader lifecycle/atomic set passed 147 in 2.84 s. All three Windows 1+3 persistence reports passed: `execution_persistence_96_single_v1/20260724T234425361716Z_b1c027b76fa7`, `execution_persistence_v1/20260724T234431041793Z_b1c027b76fa7`, and `execution_persistence_384_single_v1/20260724T234448641416Z_b1c027b76fa7`; final v2 sizes were 925, 1,585, and 3,121 bytes, all with zero hot-path reads/full rebuilds and unchanged durability. The accelerated 96-well UI report `virtual_print_array_96_v1/20260724T234536876929Z_b1c027b76fa7` passed in 5.710 s with a 1,132-byte snapshot and 108,672 cumulative serialized bytes. Both permitted Windows 384x10 attempts failed closed on existing resume-file `os.replace` access denial at completions 301 and 656; progress remained 11,082 bytes and only 3.34/7.27 MB had been serialized. Pi `192.168.0.33` passed clean-commit preflight and traced 96-well safety proof, then completed all 3,840 operations in 852.155 s. Its terminal bundle was completed and valid with no workflow errors/dialogs/starvation, 3,840 cached updates, zero full rebuilds/reads/resume loads, exact 15,369 guards, 11,520/3,840 resume/progress durable calls, max/final intents 2/0, an 11,082-byte v2 snapshot versus 590,982-byte v1 equivalent (98.12% reduction), and 42,554,880 cumulative progress bytes below the 80 MB gate. The canonical report and full screenshots/events/stacks are retained at `verification_reports/pi_compact_384x10_artifacts_20260724/`; completed and midpoint screenshots visibly show the real 16x24 UI and simulation banner. Final classification remains `fail` only because maximum event-loop and active pressure-render intervals were 3,649.251 and 3,824.153 ms (p99 lateness 186.678 ms); stacks include pass-start revision-history validation and experiment-guidance full rebuilds. Full pytest passed 3,468 with 24 skips and 148 existing warnings in 610.39 s; the validated temp root was removed, ignore checks and report-v1 validation passed. The compact representation and recovery/durability behavior are verified, but the remediation record is not labeled fully closed because the approved stress responsiveness gate did not pass. Slice 8 remains deferred. |

| 2026-07-25 | Performance remediation | pass-start transaction/history cache `not_started` -> `in_progress` | Starting commit `e76e99399edac7e845ce089de6bfb5fdfebc56a1` had a clean tracked worktree; the inaccessible pre-existing pytest temporary directories remain untouched. Call path: real UI start -> Controller authoritative preflight and existing safety checks -> consolidated pass preparation -> identity/revision guard -> in-memory successor validation -> unchanged atomic plan/progress/resume writes -> cached bundle advance -> command dispatch. Scope is limited to Controller, Model, authoritative load/revision/progress helpers, scenario diagnostics, focused tests, README, schema, and this record. Guidance rendering is measured but unchanged; Windows resume rename retries, per-well persistence, firmware, protocol, motion, pressure, and simulator behavior remain out of scope. Main risks are stale cached history, overwrite after an external edit, cache advancement after partial durability, and changed recovery semantics. Immutable successor validation, pre-write/post-write identity checks, post-success cache advancement, failure injection, explicit repair, and terminal full validation mitigate them. Rollback reverts the production cache commit; on-disk schemas require no migration. |
| 2026-07-25 | Performance remediation | pass-start instrumentation milestone complete | Commit `ea2253bc04bb` adds per-pass total/inclusive/exclusive timings, authoritative read and durable-operation deltas, revision/cache/checkpoint evidence, event-loop correlation, and guidance snapshot/rebuild timing without changing production behavior. Reduced real-UI and metrics validation passed 28 tests; the report-v1 envelope and comparison policy v1 remain unchanged. The instrumentation-only Pi 384x10 report is retained at `verification_reports/pi_pass_start_baseline_20260725/20260725T011328848631Z_ea2253bc04bb/`. It completed all 3,840 operations with exact 11,520/3,840 resume/progress durable calls, but correctly failed the stress gate: ten pass starts took 2.131-2.592 s (2.286 s median), performed 687 authoritative reads including 492 immutable-revision reads and 21 full refreshes, and correlated with a 3,666.273 ms maximum event-loop gap and 4,156.623 ms active pressure-render interval. The collector retained the failed run rather than producing a comparison set; it was copied directly and not rerun. |
| 2026-07-25 | Performance remediation | guarded pass-start cache implementation complete; terminal responsiveness blocker retained | Commit `050ae2fdf9e2` adds pure incremental successor validation, strict schema-v1/v2 progress revision retargeting, a single-use guarded preflight snapshot, consolidated `prepare_authoritative_print_pass`, in-memory bundle/session advancement, Controller routing with compatible fallback, named durable phases, and focused conflict/partial-commit/recovery tests. Focused validation passed 164 tests in 14.36 s; the real 96-well scenario passed after updating the intentional two pass-start guards. Same-Pi candidate evidence at `verification_reports/pi_pass_start_candidate_20260725/20260725T012942071298Z_050ae2fdf9e2/` completed 3,840/3,840 with a valid completed terminal plan, zero workflow errors/starvation/hot-path reads/resume loads, 3,840 cached progress updates, max/final intents 2/0, exact 11,520/3,840 resume/progress durability, and restored observers. All ten pass starts used `cached_noop`, took 98.840-119.947 ms (100.613 ms median), performed zero old-revision reads and zero full refreshes, and kept guidance rebuilds below 104.153 ms. The intentional preflight/preparation guards raise the deterministic total to 15,380. Overall stress classification remains `fail` because the required terminal recovery/full validation, outside the measured pass-start totals, correlated with a 3,439.398 ms event-loop gap and 1,885.964 ms active pressure-render interval; scheduling-lateness p99 was 180.627 ms. Final full pytest passed 3,474 tests with 24 skips and 148 existing warnings in 421.34 s. This remediation is functionally complete, but the record is not labeled fully verified because the approved active-pressure gate remains unmet. Terminal transaction optimization is the next separate performance target; guidance rebuilding and Windows resume rename contention remain separately bounded work. Slice 8 remains deferred. |
| 2026-07-25 | Performance remediation | evidence review complete | Both retained Pi `report.json` files passed schema-v1 validation. Baseline report SHA-256 is `0cfe0e85cf99b30688cfd1e1588a1bcef0f6947aa7b08a7f3eae8aecbdd8cd33`; candidate report SHA-256 is `defabb71455af8e3e6507423740505f8a6297e8b344bb8c0610725275c88bffe`. Each retained four nonempty screenshots and a bounded event trace ending at the completed milestone; the candidate completed screenshot visibly shows the simulation banner, real 16x24 plate, all wells complete, and ten-stock guidance. `verification_reports/` remains ignored, `git diff --check` passed, and the validated full-suite temp root was removed. The inaccessible pre-existing pytest directories remain untouched. |
| 2026-07-25 | Performance remediation | terminal completion transaction `not_started` -> `in_progress` | Starting commit `a81410e877e7287dd855486d499dda7ff1ee57a4` had a clean tracked worktree; the inaccessible pre-existing pytest directories remained untouched. Call path: Controller final array completion -> exact-target eligibility -> guarded active session -> in-memory completed successor/progress/resume synchronization -> immutable revision/current plan/progress/resume durable writes -> one full post-commit validation -> runtime deactivation -> idle UI. Scope was limited to Model completion, scenario diagnostics, focused terminal/system/metrics tests, README, schema, and this record. Hard abort, missing-session recovery, per-well durability, Windows rename handling, simulator, firmware, protocol, motion, and pressure behavior remained unchanged. Main risks were exposing an unvalidated terminal state, overwriting external edits, and weakening partial-commit recovery; two guards, post-validation installation, boundary failure injection, and explicit retry tests mitigate them. |
| 2026-07-25 | Performance remediation | terminal instrumentation and cached completion implemented | Instrumentation commit `91221d6` adds terminal total/nested/exclusive timings, I/O deltas, correlated gaps, summary text, and distinct post-commit validation attribution. Production/test commit `5577c13` adds generic guarded transition primitives and the completion-only cached transaction. It preserves immutable revision -> current plan -> progress -> resume ordering, performs exactly one full post-commit inspection, skips unchanged CSV exports, invalidates on conflicts/failures, and leaves abort/fallback recovery unchanged. Focused validation passed 182 tests in 23.98 s; the reduced multi-stock report passed with a 20.404 ms terminal transaction, one full validation, four fsync/replace pairs, and unchanged exports. Failure injection at every write/acceptance/validation boundary proved explicit retry repairs to a valid terminal bundle. |
| 2026-07-25 | Performance remediation | Pi terminal characterization and scaling corrections complete; final stress rerun deferred | Three clean, fail-closed Pi 384x10 diagnostic candidates were retained beneath `verification_reports/pi_terminal_candidate_20260725/` and completed all 3,840 stock/well operations with valid completed terminal bundles, zero hot-path reads/resume loads, exact 15,382 guards, 11,520/3,840 per-well resume/progress durability, max/final intents 2/0, one terminal refresh, and four terminal fsync/replace pairs. At `5577c13`, the transaction was 799.403 ms but a duplicate full target check contributed to a 1,289.610 ms gap. Commit `f70ac19` used cached eligibility, revealing that dataclass structural equality still traversed the full 384x10 plan. Commit `d31b6d8` replaced it with constant-time plan ID/revision/state checks: terminal work fell to 549.023 ms, full validation to 374.419 ms, Controller finalization to 945.079 ms, and the measured gap to 1,039.285 ms; all ten pass starts were below 272 ms and scheduling-lateness p99 was 179.011 ms. Report SHA-256 values are `105f1f97c7b7e2a5e89cf80248cd0479eb9b7ade94b7a1c10d15963b8957af66`, `dc2a9f3ecc5ebf55cd6392a35e3f1537a99e2f8083fb78083aba51b0d6b8c1bc`, and `5afe67e0203d04de099523e1b04487e72e8655873bb07daaa254367a0ca79765`. Review showed the remaining approximately 385 ms between the transaction and Controller return was tooling overhead from copying/aggregating more than 15,000 retained I/O samples. Tooling commit `ffdb5ac` replaces live boundary aggregation with constant-time cumulative counters while preserving raw report samples; 26 focused tests passed in 7.19 s. The final full suite passed 3,488 tests with 24 skips and 148 existing warnings in 420.93 s. Because no fourth 15-minute Pi run was launched after the tooling-only correction, the last raw report retains a conservative `fail` classification (1,039.285 ms gap and 1,698.431 ms pressure interval); functional/durability/terminal-transaction goals are met, but the strict end-to-end responsiveness closure remains `in_progress`. Windows resume rename contention and any future background validation remain separate. Slice 8 remains deferred. |
...
The cached terminal completion transaction is implemented and regression
verified. On the reviewed Pi 384x10 evidence, production terminal work is
549 ms, including a 374 ms mandatory full post-commit validation, and
Controller finalization is 945 ms. The transaction performs no pre-commit
authoritative JSON reads, reads every immutable revision once during the one
post-commit inspection, preserves four terminal durable writes, and leaves
hard abort/recovery conservative.

The final raw Pi classification remains a conservative failure because the
scenario itself copied and aggregated its complete I/O history before
returning to Qt. That O(completion-count) observer cost has been replaced with
constant-time counters and fully regression tested, but was not followed by a
fourth 15-minute Pi run. A future characterization may rerun one clean
384x10 candidate to close the strict gap/pressure gates; it does not require
more production persistence changes. After that, the next independent
performance targets are experiment-guidance refresh work and Windows
`execution_resume.json` atomic-rename contention. Any next change must
preserve compact intent-bound progress, four guarded durable writes per
completion, deterministic version-specific JSON, real flush/fsync/atomic
replace, fail-closed external-change behavior, and explicit restart recovery.
Slice 8 remains `deferred`.
