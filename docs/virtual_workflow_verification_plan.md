...
The bounded-resume implementation and regression validation are complete.
Preserve the one-identity-guard-per-write invariant, three resume writes plus
one progress write per completion, fail-closed external-change behavior, and
terminal full validation. The only unmet formal close gate is the reproducible
secondary Pi progress-write warning, attributed to the unchanged real
progress-phase `fsync`; do not weaken durability or fold a progress snapshot
redesign into this change. The next decision is either a separately reviewed
progress-snapshot/storage remediation or an explicit reviewed exception that
closes this bounded-resume record on its passing functional, primary,
boundedness, and durability evidence. Slice 8 remains `deferred`.
