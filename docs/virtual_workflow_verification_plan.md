...
The bounded-resume remediation is in progress. Preserve the read-elimination
remediation's one-identity-guard-per-write invariant, three resume writes plus
one progress write per completion, fail-closed external-change behavior, and
terminal full validation. Verify bounded checkpoint size and zero retained
terminal intents on all three persistence workloads and the real-UI scenario
before closing it. Slice 8 remains `deferred`; do not begin it as part of this
remediation.
