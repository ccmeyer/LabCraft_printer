...
The pass-start history-cache remediation is functionally complete. On the
reviewed same-Pi 384x10 pair, median pass startup improved from 2.286 seconds
to 0.101 seconds; all ten optimized starts stayed below 120 ms with zero
immutable-history reads and zero full bundle refreshes. Guidance rebuilds also
stayed below 105 ms. The remaining large gap is now isolated to terminal
completion: the intentionally retained recovery/full authoritative validation
correlated with a 3.44-second event-loop gap and 1.89-second active
pressure-render interval. The next focused remediation should instrument and
optimize that terminal transaction without weakening its full-validation and
crash-recovery authority. Windows separately retains
`execution_resume.json` atomic-rename contention. Any next change must preserve
the compact intent-bound progress update, four guarded durable writes per
