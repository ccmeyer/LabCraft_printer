...
Complete the instrumentation-only progress-snapshot milestone, retain its
same-Pi evidence, then implement and validate the guarded copy-on-write
authoritative construction path. Preserve one identity guard per write, three
resume writes plus one progress write per completion, schema-v1 four-space
JSON, real flush/fsync/atomic replace, fail-closed external-change behavior,
and terminal full validation. Compare non-durable construction separately
from storage variation. Slice 8 remains `deferred`.
