...
The progress-snapshot construction remediation is verified. Compact
authoritative progress schema v2 is currently being validated. The opt-in
...
writes plus one progress write per completion, version-specific deterministic
JSON, real
...
variable synchronous `fsync`; an ordered persistence worker requires a
separate plan and recovery review.
Slice 8 remains `deferred`.
